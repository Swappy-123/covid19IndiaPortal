...
app.post('/login/', async (request, response) => {
  ...
  if (dbUser !== undefined) {
    ...
    if (passMatch === true) {
      ...
      const jwtToken = jwt.sign(payload, 'swapna_secret_key')
...
function authenToken(request, response, next) {
  let jwtToken
  ...
  if (jwtToken !== undefined) {
    jwt.verify(jwtToken, 'swapna_secret_key', async (error, payload) => {
      if (error) {
        response.status(401)
...
app.get('/states/', authenToken, async (request, response) => {
...
app.get('/states/:stateId/', authenToken, async (request, response) => {
...
app.post('/districts/', authenToken, async (request, response) => {
...
app.get('/districts/:districtId/', authenToken, async (request, response) => {
...
app.put('/districts/:districtId/', authenToken, async (request, response) => {
  const {districtId} = request.params
  const {districtName, stateId, cases, cured, active, deaths} = request.body
...
app.get('/states/:stateId/stats/', authenToken, async (request, response) => {
  const {stateId} = request.params
